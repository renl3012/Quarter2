...
public class BreakoutLeft extends Canvas implements KeyListener, Runnable
{
	private Ball ball;
	...
	public BreakoutLeft()
	{
		...
		paddle = new Paddle(760, 300);
		
		bricks = new Block[5][4];
		for(int i = 0; i <= 4; i++){
			for (int h = 0; h <= 3; h++){
				bricks[i][h] = new Block (h*30, i*100, 20, 80);
			}
		}
	   ...
	   public void paint(Graphics window)
	   {
			...
			for(int i = 0; i <= 4; i++){
				for (int h = 0; h <= 3; h++){
					...
					if((ball.getX() <= bricks[i][h].getX() +bricks[i][h].getWidth() + Math.abs(ball.getXSpeed()))
							&&
			...
			if(ball.didCollideLeft(paddle) && (ball.didCollideTop(paddle) || ball.didCollideBottom(paddle))){
